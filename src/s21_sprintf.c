 ...
 * @param format Format string specifying how the data should be formatted.
 * @param ... Additional arguments corresponding to the format string.
 * @return The number of characters written to the string (excluding the null
 * terminator).
 */
int s21_sprintf(char *str, const char *format, ...) {
  ...
  while (*format) {
    if (*format == '%') {
      format++;
      format += first_init(format, &sp, ptr);
      {
        char buf[4096] = "";
        sp.count = count;
...
// Выцепляем послепроцентие и работаем с ним
int first_init(const char *format, Specif *sp, va_list ptr) {
  ...
  init_struct(buf, sp, ptr);
  free(buf);
  return a;
}

// Работаем с послепроцентием
int init_struct(char *str, Specif *sp, va_list ptr) {
  char *flag = "-+ #0";
  char *modif = "hlL";
  Specif qwer = {0};
  qwer.precision = 1;
...
// Определяем точность и ширину вывода
int precis_width(char *str, Specif *sp, va_list ptr) {
  int step = 0;
  if ((*str >= '0' && *str <= '9') ||
      *str == '*') {  // если цифра или звездочка
    if (*str == '*') {
      sp->width = va_arg(ptr, int);
    } else {
      step += parse_to_int(str,
                           &(sp->width));  // то забираю все цифры в число width
      str += step;
    }
  }
  if (*str == '.') {  // если точка то после число точности
    ...
    if (*str == '*') {
      ...
      if ('9' >= *str && '0' <= *str) {
        int by = parse_to_int(
            str,
            &(sp->precision));  // то забираю все цифры в число precision
        str += by;
        step += by;
      } else {
        sp->precision = 0;
...
int choice_format_case(char *buf, Specif sp) {
  ...
  if (s21_strchr("XGE", sp.spec)) {  // Косяк тут !!!!!!
    for (s21_size_t i = 0; buf[i]; i++) {
...
int spec_g(char *str, Specif *sp, va_list ptr) {  // МБ здесь проблема
  int count = 0;
  long double a = get_value_mod_doub(*sp, ptr);
  if (isnan(a)) {
    if (a < 0) {
...
int get_cifer(char *str, __int128_t a, int notation) {
  int count = 0;
  if (9 < a) {
    count += get_cifer(str, a / notation, notation);
    str += count;
  }
  int buf = a % notation;
  *str = buf + '0';
...
int get_cifer_u(char *str, long long unsigned a, int notation) {
  int count = 0;
  if ((long long unsigned)(notation - 1) < a) {
    count += get_cifer_u(str, a / notation, notation);
    str += count;
  }
  int buf = a % notation;
...
int unsigned_to_string(char *str, long long unsigned a, Specif sp) {
  ...
  if (0 == a) {
    *str = '0';
    ...
    int step = get_cifer_u(str, a, notation);
    str += step;
    count += step;
  }
  return count;
