...
/*In Make test failures are forced to procede*/

/*dtype might affect sscanf and sprintf results (according to specifier)*/
...
START_TEST(test_sprintf_G) {
  ...
  ck_assert_str_eq(origin, copy);  // FAIL : не переводит регистр экспоненты
...
START_TEST(test_sscanf_x) {
  ...
  sscanf(str, "Hello. My name -343.5e-2 is Jimmy Pop and I am %X years old.",
         &d2);
  ck_assert_int_eq(d1, d2);  // FAIL : не читает 16-тичные числа
  s21_sscanf(str,
             "Hello. My name -343.5e-2 is Jimmy Pop and I am %2x years old.",
             &d1);
  sscanf(str, "Hello. My name -343.5e-2 is Jimmy Pop and I am %2x years old.",
